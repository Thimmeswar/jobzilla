...
$(document).ready(function() {
    $('#delete-dash-profile .outline-primary').on('click', function() {
        var adminId = $('#delete-dash-profile').data('admin-id');
        
        if (!adminId) {
            alert('Admin ID is not set.');
            return;
        }

        $.ajax({
            url: 'Delete_admin_profile.php',
            type: 'POST',
            data: { id: adminId },
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    window.location.href = 'login.php'; // Redirect to the login page after deletion
                } else {
